...
{
    /// <summary>
    /// Implements the Excel SYD() function.
    /// SYD(cost, salvage, life, per) - sum-of-years' digits depreciation.
    /// </summary>
    public class Syd : NumericFunction
    {
        protected override double Eval(ValueEval[] args, int srcCellRow, int srcCellCol)
        {
            ...
            double per = FinancialHelper.GetDoubleArg(args, 3, srcCellRow, srcCellCol);

            double result = EFF.Financial.Syd(cost, salvage, life, per);
            CheckValue(result);
            return result;
        }
    }
