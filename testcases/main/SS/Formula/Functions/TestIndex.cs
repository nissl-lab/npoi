...
public class TestIndex  {
	...
	private static double invokeAndDereference(ValueEval[] args) {
		...
		Assert.AreEqual(NumberEval.class, ve.GetType());
	...
	public void TestMissingArg() {
		...
		try {
			...
			if (e.GetMessage().Equals("Unexpected arg eval type (NPOI.hssf.Record.Formula.Eval.MissingArgEval")) {
		...
		Assert.AreEqual(NumberEval.class, actualResult.GetType());
		Assert.AreEqual(26.0, ((NumberEval)actualResult).GetNumberValue(), 0.0);
	}
