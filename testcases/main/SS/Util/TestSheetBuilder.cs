...
namespace TestCases.SS.Util
{
	...
	new object[]{     "one", "two", "=A1+B2"}
        ...
        public void TestNotCreateEmptyCells()
        {
            ...
            ClassicAssert.AreEqual(sheet.PhysicalNumberOfRows, 3);

            NPOI.SS.UserModel.IRow firstRow = sheet.GetRow(0);
            NPOI.SS.UserModel.ICell firstCell = firstRow.GetCell(0);

            ClassicAssert.AreEqual(firstCell.CellType, CellType.Numeric);
            ClassicAssert.AreEqual(1.0, firstCell.NumericCellValue, 0.00001);


            NPOI.SS.UserModel.IRow secondRow = sheet.GetRow(1);
            ClassicAssert.IsNotNull(secondRow.GetCell(0));
            ClassicAssert.IsNull(secondRow.GetCell(2));

            NPOI.SS.UserModel.IRow thirdRow = sheet.GetRow(2);
            ClassicAssert.AreEqual(CellType.String, thirdRow.GetCell(0).CellType);
            String cellValue = thirdRow.GetCell(0).StringCellValue;
            ClassicAssert.AreEqual(testData[2][0].ToString(), cellValue);

            ClassicAssert.AreEqual(CellType.Formula, thirdRow.GetCell(2).CellType);
            ClassicAssert.AreEqual("A1+B2", thirdRow.GetCell(2).CellFormula);
        }
        [Test]
        public void TestEmptyCells()
        {
            ...
            ClassicAssert.IsNotNull(emptyCell);
            ClassicAssert.AreEqual(CellType.Blank, emptyCell.CellType);
        }
        ...
        public void TestSheetName()
        {
            ...
            ClassicAssert.AreEqual(sheetName, sheet.SheetName);
        }
    }
}