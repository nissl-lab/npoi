 ...
 */
    ...
    private static void assertWriteAndReadBack(HSSFWorkbook wb){
        ...
        for(int i = 0; i < wb.GetNumberOfSheets(); i++){
            ...
            if(info != null) {
                aggs.Put(i, info);
                HSSFPatriarch p = sheet.DrawingPatriarch();
                ...
                Assert.AreEqual("different size of raw data ande aggregate.Serialize()", dgBytes1.Length, dgBytes2.Length);
                Assert.IsTrue("raw Drawing data ("+dgBytes1.Length+" bytes) and aggregate.Serialize() are different.",
        ...
        if(aggs.Count != 0){
            ...
            for(int i = 0; i < wb2.GetNumberOfSheets(); i++){
                ...
                if(info1 != null) {
                    ...
                    Assert.AreEqual("different size of Drawing data before and After save", dgBytes1.Length, dgBytes2.Length);
                    Assert.IsTrue("drawing data ("+dgBytes1.Length+" bytes) before and After save is different.",
                            Arrays.Equals(dgBytes1, dgBytes2));
                }
            }
        }
    ...
    public void TestIncompleteData(){
        ...
        Assert.AreEqual("data was not fully Read", buffer.Length, pos);

        // serialize to byte array
        MemoryStream out = new MemoryStream();
        try {
            foreach(EscherRecord r in records) out.Write(r.serialize());
        } catch (IOException e){
            throw new RuntimeException(e);
        }
        Assert.AreEqual(HexDump.ToHex(buffer, 10), HexDump.ToHex(out.ToArray(), 10));
    }
    ...
    public void TestSolverContainerMustBeSavedDuringSerialization() {
        ...
        List<RecordBase> dgRecords = records.GetRange(19, 22);
        byte[] dgBytes = ToArray(dgRecords);
        sh.DrawingPatriarch();
        EscherAggregate agg = (EscherAggregate) ish.FindFirstRecordBySid(EscherAggregate.sid);
        Assert.AreEqual(agg.GetEscherRecords().Get(0).GetChildRecords().Count, 3);
        Assert.AreEqual(agg.GetEscherRecords().Get(0).GetChild(2).GetRecordId(), EscherContainerRecord.SOLVER_CONTAINER);
        wb = HSSFTestDataSamples.WriteOutAndReadBack(wb);
        sh = wb.GetSheetAt(0);
        sh.DrawingPatriarch();
        ...
        Assert.AreEqual("different size of Drawing data before and After save", dgBytes.Length, dgBytesAfterSave.Length);
        Assert.IsTrue("drawing data before and After save is different", Arrays.Equals(dgBytes, dgBytesAfterSave));
    }
    ...
    public void TestFileWithTextbox() {
        ...
        List<RecordBase> records = ish.Records();
        // records to be aggregated
        List<RecordBase> dgRecords = records.GetRange(19, 23);
        byte[] dgBytes = ToArray(dgRecords);
        sh.DrawingPatriarch();
    ...
    public void TestFileWithCharts(){
        ...
        List<RecordBase> records = ish.Records();
        // records to be aggregated
        List<RecordBase> dgRecords = records.GetRange(19, 21);
        byte[] dgBytes = ToArray(dgRecords);
        sh.DrawingPatriarch();
    ...
    public void Test45129() {
        ...
        Assert.AreEqual("wrong size of sheet records stream", 394, records.Count);
        // the last record before the Drawing block
        Assert.IsTrue(
                "records.Get(18) is expected to be RowRecordsAggregate but was " + records.Get(18).GetType().GetSimpleName(),
                records.Get(18) is RowRecordsAggregate);

        // records to be aggregated
        List<RecordBase> dgRecords = records.GetRange(19, 389);
        // collect Drawing records into a byte buffer.
        byte[] dgBytes = ToArray(dgRecords);
        ...
        Assert.IsTrue(
                ...
                records.Get(389) is WindowTwoRecord);
        ...
        Assert.AreEqual("wrong size of the aggregated sheet records stream", 25, records.Count);
        Assert.IsTrue(
                "records.Get(18) is expected to be RowRecordsAggregate but was " + records.Get(18).GetType().GetSimpleName(),
                records.Get(18) is RowRecordsAggregate);
        Assert.IsTrue("records.Get(19) is expected to be EscherAggregate but was " + records.Get(19).GetType().GetSimpleName(),
                records.Get(19) is EscherAggregate);
        Assert.IsTrue("records.Get(20) is expected to be Window2 but was " + records.Get(20).GetType().GetSimpleName(),
                records.Get(20) is WindowTwoRecord);
    ...
    public void TestSerializeDrawingBigger8k() {
        ...
        // the sheet's Drawing is not aggregated
        Assert.AreEqual("wrong size of sheet records stream", 32, records.Count);
        // the last record before the Drawing block
        Assert.IsTrue(
                "records.Get(18) is expected to be RowRecordsAggregate but was " + records.Get(18).GetType().GetSimpleName(),
                records.Get(18) is RowRecordsAggregate);
        ...
        // records to be aggregated
        List<RecordBase> dgRecords = records.GetRange(19, 26);
        foreach (RecordBase rb in dgRecords) {
            Record r = (Record) rb;
            short sid = r.Sid;
            // we expect that Drawing block consists of either
        ...
        Assert.IsTrue(
                ...
                records.Get(26) is WindowTwoRecord);
        ...
        Assert.AreEqual("wrong size of the aggregated sheet records stream", 26, records.Count);
        Assert.IsTrue(
                "records.Get(18) is expected to be RowRecordsAggregate but was " + records.Get(18).GetType().GetSimpleName(),
                records.Get(18) is RowRecordsAggregate);
        Assert.IsTrue("records.Get(19) is expected to be EscherAggregate but was " + records.Get(19).GetType().GetSimpleName(),
    ...
    public void TestSerializeDrawingBigger8k_noAggregation() {
        ...
        for (int i = 0; i < records.Count; i++) {
            ...
            Assert.AreEqual(r1.GetRecordSize(), r2.GetRecordSize());
            if (r1 is Record) {
                Assert.AreEqual(((Record) r1).Sid, ((Record) r2).Sid);
                Assert.IsTrue(Arrays.Equals(((Record) r1).Serialize(), ((Record) r2).Serialize()));
            }
        }
    }
    ...
    public void TestSerializeDrawingWithComments()  {
        ...
        // the sheet's Drawing is not aggregated
        Assert.AreEqual("wrong size of sheet records stream", 46, records.Count);
        // the last record before the Drawing block
        Assert.IsTrue(
                "records.Get(18) is expected to be RowRecordsAggregate but was " + records.Get(18).GetType().GetSimpleName(),
                records.Get(18) is RowRecordsAggregate);
        ...
        // records to be aggregated
        List<RecordBase> dgRecords = records.GetRange(19, 39);
        foreach (RecordBase rb in dgRecords) {
            Record r = (Record) rb;
            short sid = r.Sid;
            // we expect that Drawing block consists of either
        ...
        Assert.IsTrue(
                ...
                records.Get(39) is WindowTwoRecord);
        ...
        Assert.AreEqual("wrong size of the aggregated sheet records stream", 27, records.Count);
        Assert.IsTrue(
                "records.Get(18) is expected to be RowRecordsAggregate but was " + records.Get(18).GetType().GetSimpleName(),
                records.Get(18) is RowRecordsAggregate);
        Assert.IsTrue("records.Get(19) is expected to be EscherAggregate but was " + records.Get(19).GetType().GetSimpleName(),
    ...
    public void TestFileWithPictures() {
        ...
        // the sheet's Drawing is not aggregated
        Assert.AreEqual("wrong size of sheet records stream", 315, records.Count);
        // the last record before the Drawing block
        Assert.IsTrue(
                "records.Get(21) is expected to be RowRecordsAggregate but was " + records.Get(21).GetType().GetSimpleName(),
                records.Get(21) is RowRecordsAggregate);

        // records to be aggregated
        List<RecordBase> dgRecords = records.GetRange(22, 300);
        foreach (RecordBase rb in dgRecords) {
            Record r = (Record) rb;
            short sid = r.Sid;
            // we expect that Drawing block consists of either
        ...
        Assert.IsTrue(
                ...
                records.Get(300) is WindowTwoRecord);
        ...
        Assert.AreEqual("wrong size of the aggregated sheet records stream", 38, records.Count);
        Assert.IsTrue(
                "records.Get(21) is expected to be RowRecordsAggregate but was " + records.Get(21).GetType().GetSimpleName(),
                records.Get(21) is RowRecordsAggregate);
        Assert.IsTrue("records.Get(22) is expected to be EscherAggregate but was " + records.Get(22).GetType().GetSimpleName(),
                records.Get(22) is EscherAggregate);
        Assert.IsTrue("records.Get(23) is expected to be Window2 but was " + records.Get(23).GetType().GetSimpleName(),
                records.Get(23) is WindowTwoRecord);
    ...
    public void TestUnhandledContinue() {
        ...
        List<Record> dgRecords = RecordFactory.CreateRecords(new MemoryStream(dgBytes));
        ...
        for (int i = 0; i < expectedSids.Length; i++) {
            Assert.AreEqual("unexpected record.sid and index[" + i + "]", expectedSids[i], dgRecords.Get(i).Sid);
        }
        ...
        Assert.AreEqual("drawing was not fully aggregated", 2, records.Count);
        Assert.IsTrue("expected EscherAggregate", records.Get(0) is EscherAggregate);
        Assert.IsTrue("expected EOFRecord", records.Get(1) is EOFRecord);
    ...
    public void TestUnhandledContinue2() {
        ...
        byte[] dgBytesAfterSave = agg.Serialize();
        Assert.AreEqual("different size of Drawing data before and After save", dgBytes.Length, dgBytesAfterSave.Length);
        Assert.IsTrue("drawing data brefpore and After save is different", Arrays.Equals(dgBytes, dgBytesAfterSave));
    }
