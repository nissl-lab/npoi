...
namespace TestCases.XSSF.EventUserModel
{
    ...
    /// <summary>
    /// Tests for <see cref="NPOI.xssf.EventUserModel.XSSFReader" />
    /// </summary>
    [TestFixture]
    public sealed class TestReadOnlySharedStringsTable
    {
        ...
        public void TestParse()
        {
            ...
            Assert.AreEqual(stbl.Count, rtbl.Count);
            Assert.AreEqual(stbl.UniqueCount, rtbl.UniqueCount);

            Assert.AreEqual(stbl.Items.Count, stbl.UniqueCount);
            Assert.AreEqual(rtbl.Items.Count, rtbl.UniqueCount);
            for(int i = 0; i < stbl.UniqueCount; i++)
            {
                CT_Rst i1 = stbl.GetEntryAt(i);
                String i2 = rtbl.GetEntryAt(i);
                Assert.AreEqual(i1.t, i2);
            }
        ...
        public void TestPhoneticRuns()
        {
            ...
            Assert.AreEqual("\u65E5\u672C\u30AA\u30E9\u30AF\u30EB \u30CB\u30DB\u30F3", rtbl.GetEntryAt(3));
        ...
        private void assertEmptySST(OPCPackage pkg)
        {
            ...
            Assert.IsNull(sst.Items); // same state it's left in if fed a package which has no SST part.
        }
