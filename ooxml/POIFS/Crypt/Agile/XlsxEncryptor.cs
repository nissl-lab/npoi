...
{
    public static class XlsxEncryptorOrig
    {
        ...
        public static void FromBytesToFile(byte[] wbByte, string outputPath, string password)
        {
            ...
            try
            {
                (XDocument xmlDoc, byte[] encryptionKey, byte[] keySalt, byte[] integritySalt) =
                    GenerateEncryptionInfo(password);
...
#pragma warning restore CA5350
        ...
        {
            using RootStorage root = RootStorage.Create(outputPath);
...
#pragma warning restore CA5350
        ...
        /// </summary>
        public static class XlsxEncryptor2
        {
            ...
            #region Internal Helper Methods (used by AgileEncryptor)

            internal static (XDocument xmlDoc, byte[] encryptionKey, byte[] keySalt, byte[] integritySalt)
...
#pragma warning restore CA5350
            ...
            internal static void UpdateIntegrityHmac(byte[] encryptedPackage, int oleStreamSize, byte[] encryptionKey,
                byte[] keySalt, byte[] integritySalt, XDocument xmlDoc)
            {
#pragma warning disable CA5350
                using HMACSHA1 hmac = new(integritySalt);
#pragma warning restore CA5350
            ...
            internal static void CreateEncryptedFile(string outputPath, XDocument encryptionInfo, byte[] encryptedData)
            {
                using RootStorage root = RootStorage.Create(outputPath);
                using(CfbStream s = root.CreateStream("EncryptedPackage"))
                {
            ...
            internal static void CreateEncryptedFile(Stream outputStream, XDocument encryptionInfo,
                byte[] encryptedData)
            {
                using MemoryStream ms = new();
            ...
            internal static byte[] EncryptPackage(byte[] packageData, byte[] encryptionKey, byte[] keySalt)
            {
                using MemoryStream ms = new();
                using BinaryWriter bw = new(ms);
                bw.Write((long) packageData.Length);
...
#pragma warning restore CA5350
                ...
                using(Aes aes = Aes.Create())
                {
                    ...
                    decryptedVerifierHash =
                        dec.TransformFinalBlock(encryptedVerifierHash, 0, encryptedVerifierHash.Length);
                }
...
namespace NPOI.POIFS.Crypt
{
    ...
    public static class XlsxEncryptor
    {
        ...
        public static (XDocument xmlDoc, byte[] encryptionKey, byte[] keySalt, byte[] integritySalt) GenerateEncryptionInfo(string password)
        {
            byte[] keySalt = RandomBytes(SaltSize); // keyData.saltValue
            byte[] verifierSalt = RandomBytes(SaltSize); // p:encryptedKey.saltValue
            byte[] pwHash = HashPassword(password, verifierSalt, SpinCount);
